...
class S2VTAttModel(nn.Module):
    ...
    def forward(self, vid_feats, target_variable=None,
        ...
        if h is not None:
            seq_prob, seq_preds, _ = self.decoder(encoder_outputs, h, target_variable, mode, opt)
        else:
            seq_prob, seq_preds, _ = self.decoder(encoder_outputs, encoder_hidden, target_variable, mode, opt)
        return seq_prob, seq_preds, encoder_hidden, _
